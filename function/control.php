...
function localStore($name_cust, $name_item, $price_item, $image_item) {
    ...
    $check = "INSERT INTO `user-item` (`name`, `item`, `price`, `image`) VALUES('$name_entry', '$item_entry', '$price_entry',  '$image_entry')";
...
function passwordreset($email) {
    ...
    if($responce_list == true ) {
        ...
        $check_again = "SELECT * FROM `users` WHERE `email` = '$email' LIMIT 1";
        ...
        if($check_result) {
            ...
            if(mysqli_num_rows($check_result) == 1) {
                ...
                $_SESSION['users']['email_reset'] = $_SESSION['users']['email'];
                $_SESSION['users']['token_reset'] = $_SESSION['users']['token'];
                $_SESSION['users']['name_reset'] = $_SESSION['users']['name'];

                require "../forgetlink.php";
...
// to change password finaly
function changepassword($password_one, $old_token, $new_token) {

    require "../database/connect.php";

    $check = "SELECT * FROM `users` WHERE `token` = '$old_token' LIMIT 1";
    ...
    if($check_result) {
        ...
        if(mysqli_num_rows($check_result) == 1) {
            ...
            $user_only = mysqli_fetch_array($check_result, MYSQLI_ASSOC);

            $name_entry = $user_only['name'];
        
            $email_entry= $user_only['email'];
                    
            $verified = $user_only['verified'];

            $hash = password_hash($password_one, PASSWORD_DEFAULT);
    
            $hash_entry = mysqli_real_escape_string($conn, $hash);  
        

            $check_again = "DELETE FROM `users` WHERE `token` = '$old_token'";
            ...
            if($check_again_result) {
                ...
                $check_finaly = "INSERT INTO `users` (`name`, `email`, `password`, `verified`, `token`) VALUES('$name_entry', '$email_entry', '$hash_entry', '$verified', '$new_token')";


                $result_finaly = mysqli_query($conn, $check_finaly);
